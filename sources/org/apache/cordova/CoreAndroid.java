...
public class CoreAndroid extends CordovaPlugin {
    ...
    public void loadUrl(String url, JSONObject props) throws JSONException {
        LOG.m7d("App", "App.loadUrl(" + url + "," + props + ")");
        int wait = 0;
    ...
    public void overrideBackbutton(boolean override) {
        LOG.m13i("App", "WARNING: Back Button Default Behavior will be overridden.  The backbutton event will be fired!");
        this.webView.setButtonPlumbedToJs(4, override);
    }

    public void overrideButton(String button, boolean override) {
        LOG.m13i("App", "WARNING: Volume Button Default Behavior will be overridden.  The volume event will be fired!");
        ...
        this.telephonyReceiver = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                if (intent != null && intent.getAction().equals("android.intent.action.PHONE_STATE") && intent.hasExtra(ServerProtocol.DIALOG_PARAM_STATE)) {
                    String extraData = intent.getStringExtra(ServerProtocol.DIALOG_PARAM_STATE);
                    if (extraData.equals(TelephonyManager.EXTRA_STATE_RINGING)) {
                        LOG.m13i(CoreAndroid.TAG, "Telephone RINGING");
                        CoreAndroid.this.webView.getPluginManager().postMessage("telephone", "ringing");
                    } else if (extraData.equals(TelephonyManager.EXTRA_STATE_OFFHOOK)) {
                        LOG.m13i(CoreAndroid.TAG, "Telephone OFFHOOK");
                        CoreAndroid.this.webView.getPluginManager().postMessage("telephone", "offhook");
                    } else if (extraData.equals(TelephonyManager.EXTRA_STATE_IDLE)) {
                        LOG.m13i(CoreAndroid.TAG, "Telephone IDLE");
    ...
    private void sendEventMessage(String action) {
        ...
        try {
            obj.put(NativeProtocol.WEB_DIALOG_ACTION, action);
        } catch (JSONException e) {
            LOG.m11e(TAG, "Failed to create event message", (Throwable) e);
        }
