...
public class CordovaBridge {
    ...
    private boolean verifySecret(String action, int bridgeSecret) throws IllegalAccessException {
        if (!this.jsMessageQueue.isBridgeEnabled()) {
            if (bridgeSecret == -1) {
                LOG.m7d(LOG_TAG, action + " call made before bridge was enabled.");
            } else {
                LOG.m7d(LOG_TAG, "Ignoring " + action + " from previous page load.");
            }
            return false;
        } else if (this.expectedBridgeSecret >= 0 && bridgeSecret == this.expectedBridgeSecret) {
            return true;
        } else {
            LOG.m10e(LOG_TAG, "Bridge access attempt with wrong secret token, possibly from malicious code. Disabling exec() bridge!");
            clearBridgeSecret();
    ...
    public String promptOnJsPrompt(String origin, String message, String defaultValue) {
        if (defaultValue != null && defaultValue.length() > 3 && defaultValue.startsWith("gap:")) {
            ...
            try {
                String r2 = jsRetrieveJsMessages(Integer.parseInt(defaultValue.substring(9)), AppEventsConstants.EVENT_PARAM_VALUE_YES.equals(message));
                if (r2 == null) {
                    return "";
                }
                return r2;
            ...
            LOG.m10e(LOG_TAG, "gap_init called from restricted origin: " + origin);
            return "";
        }
    }
}
