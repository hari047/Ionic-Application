...
public class PluginManager {
    ...
    public void exec(String service, String action, String callbackId, String rawArgs) {
        ...
        if (plugin == null) {
            LOG.m7d(TAG, "exec() call to unknown plugin: " + service);
        ...
        try {
            ...
            if (duration > ((long) SLOW_EXEC_WARNING_THRESHOLD)) {
                LOG.m19w(TAG, "THREAD WARNING: exec() call to " + service + "." + action + " blocked the main thread for " + duration + "ms. Plugin should use CordovaInterface.getThreadPool().");
            }
            if (!wasValidAction) {
            ...
            LOG.m11e(TAG, "Uncaught exception from plugin", (Throwable) e2);
