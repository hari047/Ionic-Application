...
public class SystemWebChromeClient extends WebChromeClient {
    ...
    public void onExceededDatabaseQuota(String url, String databaseIdentifier, long currentQuota, long estimatedSize, long totalUsedQuota, QuotaUpdater quotaUpdater) {
        LOG.m9d(LOG_TAG, "onExceededDatabaseQuota estimatedSize: %d  currentQuota: %d  totalUsedQuota: %d", Long.valueOf(estimatedSize), Long.valueOf(currentQuota), Long.valueOf(totalUsedQuota));
        quotaUpdater.updateQuota(this.MAX_QUOTA);
    }

    public void onConsoleMessage(String message, int lineNumber, String sourceID) {
        if (VERSION.SDK_INT == 7) {
            LOG.m9d(LOG_TAG, "%s: Line %d : %s", sourceID, Integer.valueOf(lineNumber), message);
    ...
    public boolean onConsoleMessage(ConsoleMessage consoleMessage) {
        if (consoleMessage.message() != null) {
            LOG.m9d(LOG_TAG, "%s: Line %d : %s", consoleMessage.sourceId(), Integer.valueOf(consoleMessage.lineNumber()), consoleMessage.message());
        }
    ...
    public void openFileChooser(final ValueCallback<Uri> uploadMsg, String acceptType, String capture) {
        ...
        this.parentEngine.cordova.startActivityForResult(new CordovaPlugin() {
            public void onActivityResult(int requestCode, int resultCode, Intent intent) {
                ...
                LOG.m7d(SystemWebChromeClient.LOG_TAG, "Receive file chooser URL: " + result);
    ...
    public boolean onShowFileChooser(WebView webView, final ValueCallback<Uri[]> filePathsCallback, FileChooserParams fileChooserParams) {
        try {
            ...
            LOG.m22w("No activity found to handle file chooser intent.", (Throwable) e);
    ...
    public void onPermissionRequest(PermissionRequest request) {
        LOG.m7d(LOG_TAG, "onPermissionRequest: " + Arrays.toString(request.getResources()));
