...
public class SystemWebViewClient extends WebViewClient {
    ...
    public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
        if (this.isCurrentlyLoading) {
            LOG.m9d(TAG, "CordovaWebViewClient.onReceivedError: Error code=%s Description=%s URL=%s", Integer.valueOf(errorCode), description, failingUrl);
            if (errorCode == -10) {
                this.parentEngine.client.clearLoadTimeoutTimer();
                if (view.canGoBack()) {
                    view.goBack();
    ...
    public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
        try {
            if (!this.parentEngine.pluginManager.shouldAllowRequest(url)) {
                LOG.m19w(TAG, "URL blocked by whitelist: " + url);
            ...
            if (!(e instanceof FileNotFoundException)) {
                LOG.m11e(TAG, "Error occurred while loading a file (returning a 404).", (Throwable) e);
            }
            return new WebResourceResponse("text/plain", "UTF-8", null);
        }
    }
