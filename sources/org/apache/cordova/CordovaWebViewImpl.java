...
public class CordovaWebViewImpl implements CordovaWebView {
    ...
    protected class EngineClient implements Client {
        ...
        public void onPageStarted(String newUrl) {
            LOG.m7d(CordovaWebViewImpl.TAG, "onPageDidNavigate(" + newUrl + ")");
        ...
        public void onPageFinishedLoading(String url) {
            LOG.m7d(CordovaWebViewImpl.TAG, "onPageFinished(" + url + ")");
            clearLoadTimeoutTimer();
        ...
        public boolean onNavigationAttempt(String url) {
            ...
            LOG.m19w(CordovaWebViewImpl.TAG, "Blocked (possibly sub-frame) navigation to non-allowed URL: " + url);
            return true;
        }
    }
    ...
    public void loadUrlIntoView(final String url, boolean recreatePlugins) {
        ...
        final Runnable loadError = new Runnable() {
            public void run() {
                ...
                LOG.m10e(CordovaWebViewImpl.TAG, "CordovaWebView: TIMEOUT ERROR!");
                JSONObject data = new JSONObject();
                try {
    ...
    public void showWebPage(String url, boolean openExternal, boolean clearHistory, Map<String, Object> map) {
        LOG.m9d(TAG, "showWebPage(%s, %b, %b, HashMap)", url, Boolean.valueOf(openExternal), Boolean.valueOf(clearHistory));
        if (clearHistory) {
        ...
        if (!openExternal) {
            if (this.pluginManager.shouldAllowNavigation(url)) {
                ...
                LOG.m19w(TAG, "showWebPage: Refusing to load URL into webview since it is not in the <allow-navigation> whitelist. URL=" + url);
            }
        }
        if (!this.pluginManager.shouldOpenExternalUrl(url).booleanValue()) {
            LOG.m19w(TAG, "showWebPage: Refusing to send intent for URL since it is not in the <allow-intent> whitelist. URL=" + url);
            return;
        }
        try {
            ...
            LOG.m11e(TAG, "Error loading url " + url, (Throwable) e);
        }
    }

    @Deprecated
    public void showCustomView(View view, CustomViewCallback callback) {
        LOG.m7d(TAG, "showing Custom View");
    ...
    public void hideCustomView() {
        if (this.mCustomView != null) {
            LOG.m7d(TAG, "Hiding Custom View");
    ...
    public void sendJavascriptEvent(String event) {
        ...
        if (this.appPlugin == null) {
            LOG.m19w(TAG, "Unable to fire event without existing plugin");
        } else {
