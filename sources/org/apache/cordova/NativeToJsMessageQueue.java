...
public class NativeToJsMessageQueue {
    ...
    public void setBridgeMode(int value) {
        if (value < -1 || value >= this.bridgeModes.size()) {
            LOG.m7d(LOG_TAG, "Invalid NativeToJsBridgeMode: " + value);
            return;
        }
        BridgeMode newMode = value < 0 ? null : (BridgeMode) this.bridgeModes.get(value);
        if (newMode != this.activeBridgeMode) {
            LOG.m7d(LOG_TAG, "Set native->JS mode to " + (newMode == null ? "null" : newMode.getClass().getSimpleName()));
            synchronized (this) {
    ...
    public void addPluginResult(PluginResult result, String callbackId) {
        if (callbackId == null) {
            LOG.m11e(LOG_TAG, "Got plugin result with no callbackId", new Throwable());
            return;
        }
    ...
    private void enqueueMessage(org.apache.cordova.NativeToJsMessageQueue.JsMessage r3) {
        /*
            ...
            org.apache.cordova.LOG.m7d(r0, r1)     // Catch:{ all -> 0x0020 }
    ...
    public void setPaused(boolean value) {
        if (this.paused && value) {
            LOG.m11e(LOG_TAG, "nested call to setPaused detected.", new Throwable());
        }
        this.paused = value;
        if (!value) {
            synchronized (this) {
